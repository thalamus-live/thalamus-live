  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/tiktokpgsid6TJGjs85B0WumjohXrTt8mRhVfB.txt') {
        ...
        if (url.pathname === '/terms') {
                const html = `<!DOCTYPE html><html lang="es"><head><meta charset="utf-8"><title>Terminos de Servicio - Esquina Radar</title><meta name="viewport" content="width=device-width, initial-scale=1"><style>body{font-family:-apple-system,Segoe UI,Roboto,sans-serif;max-width:760px;margin:40px auto;padding:0 20px;line-height:1.6;color:#222}h1{font-size:26px}h2{font-size:19px;margin-top:32px}p,li{font-size:15px}</style></head><body><h1>Terminos de Servicio</h1><p><strong>Esquina Radar</strong> - Ultima actualizacion: 18 de julio de 2026</p><h2>1. Aceptacion de los terminos</h2><p>Al usar Esquina Radar aceptas estos Terminos de Servicio. Si no estas de acuerdo, no uses la aplicacion.</p><h2>2. Descripcion del servicio</h2><p>Esquina Radar es una herramienta de analisis estadistico de futbol en tiempo real (corners, goles, probabilidades) con fines informativos y de entretenimiento. No es un servicio de apuestas ni facilita transacciones de dinero.</p><h2>3. Uso permitido</h2><ul><li>Debes ser mayor de 18 anios para usar esta aplicacion.</li><li>El contenido generado es informativo y no constituye asesoria financiera ni garantia de resultados.</li><li>Esta prohibido usar la app para actividades ilegales o para automatizar spam en TikTok u otras plataformas.</li></ul><h2>4. Integracion con TikTok</h2><p>Al conectar tu cuenta de TikTok autorizas a Esquina Radar a publicar contenido de video generado automaticamente en tu nombre, segun la configuracion que definas en la app. Puedes revocar este acceso en cualquier momento.</p><h2>5. Exactitud de la informacion</h2><p>Las estadisticas y predicciones se generan a partir de fuentes publicas de datos deportivos y modelos internos. No garantizamos exactitud al cien por ciento ni nos hacemos responsables de decisiones tomadas con base en esta informacion.</p><h2>6. Limitacion de responsabilidad</h2><p>Esquina Radar se ofrece tal cual, sin garantias de ningun tipo. No somos responsables por perdidas economicas derivadas del uso de la informacion proporcionada por la app.</p><h2>7. Cambios en el servicio</h2><p>Nos reservamos el derecho de modificar o discontinuar funciones de la app en cualquier momento.</p><h2>8. Contacto</h2><p>Para consultas sobre estos terminos escribenos a: panfernan5@gmail.com</p></body></html>`;
                return new Response(html, { headers: { 'Content-Type': 'text/html; charset=utf-8' } });
        }
